...
//     }, 0);
//     if(this.FormData.actualAmount > this.FormData.discountAmount){
//       this.FormData.totalOrder = this.FormData.actualAmount -  this.FormData.discountAmount;
//     }else{
//       this.toastr.error("The discount amount cannot exceed the actual amount.", "Error");
...
allProduct:any[]=[];
  ...
  getProductList(){
    this.dataService.GetData('Products/GetAllProductData?take=10000&skip=0').subscribe((data:any)=>{
      this.allProduct = data.data
    })
  }
...
getDiscountandActualAmount(productId: any, pharmacyId: any): Promise<{ actualAmount: number, discountAmount: number }> {
  return new Promise((resolve, reject) => {
    this.dataService.GetData(`Settings/GetProductStockPriceDiscountPrice?productId=${productId}&pharmacyId=${pharmacyId}`)
      .subscribe(
        (data: any) => {
          if (data.data) {
            resolve({
              actualAmount: data.data.purchaseRate,
              discountAmount: data.data.discountPrice
            });
          } else {
            resolve({ actualAmount: 0, discountAmount: 0 }); // Default values
          }
        },
        (error) => reject(error) // Handle API errors
      );
  });
}
  ...
  onValueReceived(eventData: { value: any; fieldName?: any }) {
    ...
    if(eventData.fieldName == 'pharmacyId'){
      ...
      this.dataService.GetData('Settings/GetPharmacyStockPriceDiscountPrice?pharmacyId='+eventData.value).subscribe((data:any)=>{
        console.log(data);
        this.pharmacyDiscount = data.data.discountPercent;
      });
    }
  }
  ...
  onRowUpdated(event: any) {
    ...
    if (updatedData.orderQty) {
      ...
      if(exist){
        ...
        exist.unitPrice = updatedData.unitPrice
        exist.totalPrice = Number(updatedData.orderQty) * updatedData.unitPrice;
        updatedData.actualAmount = updatedData.orderQty * updatedData.unitPrice;
        updatedData.discountAmount = updatedData.orderQty * updatedData.discountPerAmount;
        updatedData.discountAmount += updatedData.actualAmount * this.pharmacyDiscount / 100;
          // Calculate and update the 'totalPrice'
      updatedData.totalPrice = updatedData.actualAmount - updatedData.discountAmount;
      } 
      this.totalAmountCalculate();
    }
